...
import (
	...
	saveDal "medblogers_base/internal/modules/admin/entities/banner/action/save_mobile_image/dal"
	"medblogers_base/internal/modules/admin/entities/banner/action/save_mobile_image/service/image"
...
type actionDal interface {
	SaveBannerMobileImage(ctx context.Context, bannerID int64, imageID, fileType string) error
}
...
func (a *Action) Do(ctx context.Context, bannerID int64, imageData []byte) (string, error) {
	...
	if err = a.actionDal.SaveBannerMobileImage(ctx, bannerID, imageID, fileType); err != nil {
		return "", err
	}

	if banner.MobileImage != "" && banner.MobileFileType != "" {
		err = a.image.DeleteImage(ctx, fmt.Sprintf("%s.%s", banner.MobileImage, banner.MobileFileType))
		if err != nil {
			return "", err
		}
	}
