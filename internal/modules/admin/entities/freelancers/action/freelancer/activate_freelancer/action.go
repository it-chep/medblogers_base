package activate_freelancer

import (
	"context"
	"github.com/pkg/errors"
	"medblogers_base/internal/modules/admin/entities/freelancers/action/freelancer/activate_freelancer/dal"
...
type ActionDal interface {
	ActivateFreelancer(ctx context.Context, freelancerID int64) (err error)
}
type Action struct {
	commonDal CommonDal
	actionDal ActionDal
...
func (a *Action) Do(ctx context.Context, freelancerID int64) error {
	...
	if frncer.GetIsActive() {
		return errors.New("Фрилансер уже активен")
	}

	return a.actionDal.ActivateFreelancer(ctx, freelancerID)
}
