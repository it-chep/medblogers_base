...
func (r *Repository) GetSpecialities(ctx context.Context) (speciality.Specialities, error) {
	sql := `
		select id, name from freelancers_speciality
		`

	var specialities dao.SpecialitiesDAO
	err := pgxscan.Select(ctx, r.db, &specialities, sql)
	if err != nil {
		return nil, err
	}
